...
export function ToolsList({ tools }: { tools: Tool[] }) {
  ...
  const query          = searchParams.get("q")?.toLowerCase().trim() ?? undefined;
  ...
  const buildHref = (overrides: Record<string, string | undefined>) => {
    ...
    if (activeLevel)    next.level    = activeLevel;
    if (query)          next.q        = searchParams.get("q")!;
  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-4 mb-8 pb-6 border-b border-border">
        <FilterRow label="分类">
          <FilterChip href={buildHref({ category: undefined })} active={!activeCategory}>全部</FilterChip>
          {categories.map((cat) => (
            <FilterChip key={cat.slug} href={buildHref({ category: cat.slug })} active={activeCategory === cat.slug}>
              {cat.emoji} {cat.name}
            </FilterChip>
          ))}
        </FilterRow>
        <FilterRow label="等级">
          <FilterChip href={buildHref({ level: undefined })} active={!activeLevel}>全部</FilterChip>
          {LEVELS.map((lvl) => (
      ...
      {filtered.length === 0 ? (
        <div className="py-20 text-center text-muted">
          ...
          <p>没有匹配的工具</p>
          <Link href="/tools" className="inline-block mt-3 text-sm text-foreground underline">清除筛选</Link>
        </div>
      ) : (
        <>
          <div className="text-sm text-muted mb-4">显示 {filtered.length} / {tools.length}</div>
  ...
  return (
    <Link
      ...
      className={`inline-flex items-center gap-1 px-3 py-1.5 text-sm rounded-md border transition-colors ${
        active ? "" : "bg-card text-muted border-border hover:text-foreground hover:border-foreground/20"
      }`}
    >
      {children}
    </Link>
